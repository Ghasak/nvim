...
local lsp_func = function()
    ...
    for _, server in ipairs(servers) do
        ...
        elseif buff_ft == "python" and
            (server.name == "pyright" or server.name == "pylsp") then --  [python] For nvim 0.51 serer.name is python , while for 0.6 it is now pyright)
        ...
        elseif buff_ft == "tex" and server.name == "texlab" or server.name ==
            "ltex" then
            return string.format("%s :latex ", server_icon)
        else
            if server.name == "" or server.name == nil or buff_ft == "" or
                buff_ft == nil then
                return string.format("%s : %-lsp*:", server_icon_not_not_known)
            else
                return string.format("%s : %s-lsp*: %s",
...
function M.setup()
    require('lualine').setup {
        options = {
            ...
            disabled_filetypes = {
                statusline = {
                    'packer', 'NvimTree', 'Telescope', 'Dashboard', 'Packer',
                    'FZF', 'Alpha'
                },
