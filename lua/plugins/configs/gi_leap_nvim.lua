...
M.setup = function()
  -- 1) Remove any previous mappings (avoid netrw conflict)
  for _, mode in ipairs { "n", "x", "o" } do
    pcall(vim.keymap.del, mode, "s")
    pcall(vim.keymap.del, mode, "S")
  end

  -- 2) Manual explicit Leap mappings (no deprecated API)
  local leap = require "leap"
  vim.keymap.set({ "n", "x", "o" }, "s", "<Plug>(leap-forward)", { desc = "Leap forward (2-char motion)" })
  vim.keymap.set({ "n", "x", "o" }, "S", "<Plug>(leap-backward)", { desc = "Leap backward (2-char motion)" })
  vim.keymap.set({ "n", "x", "o" }, "gs", "<Plug>(leap-from-window)", { desc = "Leap across windows" })

  -- 3) Custom fancy highlighting tweaks
  leap.opts.highlight_unlabeled_phase_one_targets = true
  leap.opts.max_phase_one_targets = 100
  vim.api.nvim_set_hl(0, "LeapBackdrop", { link = "Comment" })

  -- optional visual offset tweak
  ...
  -- 4) Clear search highlight before leap
  vim.keymap.set(
    "n",
    "s",
    ":silent! noh<CR><Plug>(leap-forward)",
    { silent = true, desc = "Leap forward (clear search)" }
  )
  vim.keymap.set(
    "n",
    "S",
    ":silent! noh<CR><Plug>(leap-backward)",
    { silent = true, desc = "Leap backward (clear search)" }
  )
...
--              leap.nvim: `set_default_mappings()` is deprecated. See `:help leap-mappings` to update your config.
--
-- local M = {}
--
-- M.setup = function()
