...
-- Initialize M.keys as empty table
M.keys = {}

function M.setup_snacks_keymaps()
  -- Check if Snacks is available
  local snacks_ok, _ = pcall(function() return Snacks end)
  if not snacks_ok then
    vim.notify("Snacks not loaded yet, skipping keymap setup", vim.log.levels.WARN)
    return
  end
  ...
  M.keys = {
    ...
    { "<leader>bs", function() Snacks.picker.smart() end, desc = "Smart Find Files" },
  ...
  vim.keymap.set("n", "<leader><leader>pa", function()
    Snacks.input({ prompt = "Session name: " }, function(name)
  ...
  -- vim.notify("✓ Snacks keymaps registered", vim.log.levels.INFO)
end
M.codeium_keys = {
  {
    "<C-g>",
