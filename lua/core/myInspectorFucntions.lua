...
vim.api.nvim_create_user_command('RELOAD', function()
    vim.pretty_print(
        "Source current file, similar effect if you hit Ctrl + Enter from the mapping above")
    vim.cmd([[source %]])
end, {nargs = 0})

vim.api.nvim_create_user_command('RELOADER', function(opts)
    vim.notify("Will load the file you change: " .. "[" .. opts.args .. "]",
               vim.log.levels.INFO)
    vim.notify(
        " use :RELOADER file_name <- file_name is your lua file without quotes",
        vim.log.levels.INFO)
...
vim.api.nvim_set_keymap('n', '<C-CR>', '', {
    noremap = true,
    callback = function()
        local file_path = vim.fn.input('Path to executable: ',
                                       vim.fn.getcwd() .. '/', 'file')
