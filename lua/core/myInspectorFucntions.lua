...
vim.api.nvim_create_user_command("RELOAD", function()
  vim.print "Source current file, similar effect if you hit Ctrl + Enter from the mapping above"
  vim.cmd [[source %]]
end, { nargs = 0 })
...
vim.api.nvim_set_keymap("n", "<C-CR>", "", {
  ...
  callback = function()
    ...
    --vim.api.nvim_exec(string.format(":RUNNER  %s", file_name), false) -- deprecated
    vim.api.nvim_exec2(string.format(":RUNNER  %s", file_name), { output = false }) -- deprecated
  end,
